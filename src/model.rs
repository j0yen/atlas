...
/// Edge kind: where the dependency was asserted.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum EdgeKind {
    /// From `**Depends on:**` frontmatter (authoritative).
    Frontmatter,
    /// From a gossip `Order:` block (best-effort).
    Gossip,
}
...
/// A typed dependency edge between two PRDs.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Edge {
    /// Dependent PRD filename (e.g. "PRD-atlas-edges.md").
    pub from: String,
    /// Dependency PRD filename (e.g. "PRD-atlas-core.md").
    pub to: String,
    /// Where the edge was asserted.
    pub kind: EdgeKind,
    /// Provenance: `file:line` where the edge was parsed from.
    pub source: String,
}
